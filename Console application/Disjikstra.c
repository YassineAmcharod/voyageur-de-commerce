...
int min_distance(int ville_depart,int* L_V)
{
    ...
    for (int k=0;k<len;k++)
    {printf("\nliste_ville[k]=%d",L_V[k]);int tmp=existe(L_V[k]);
...
void ER_2A(int* L_V)/// prob au niveau de pointeur L_V ou C_opt
{
    printf("ville_pre==>%d ||len==>%d\n",ville_pre,len);
    int* C_opt;//C_opt= (int*)calloc(len+2,sizeof(int));
    printf("\nListe_ville:");
    for (int r=0;r<len;r++) printf("||-->%d",L_V[r]);
    C_opt=Disjiksta(L_V);printf("\nChemin_EA_2A:");
